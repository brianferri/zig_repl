...
test "compliance: diagnostics match the compiler's wording" {
    ...
    // An AstGen error on a `var`-decl name must still surface when it leaves the ZIR
    // empty: the var-advisory suppression only applies to an analysable ZIR.
    try compliance.expectDiagnostic(a, &.{"var @\"\\m\";"}, "invalid escape character");
}
