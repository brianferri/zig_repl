...
test "compliance: methods compose -- call each other and take struct args" {
    try compliance.check(a, .{
        .{ .src = &.{"blk: { const V = struct { n: u8, fn base(self: @This()) u8 { return self.n; } fn p1(self: @This()) u8 { return self.base() + 1; } fn p2(self: @This()) u8 { return self.p1() + 1; } }; const v: V = .{ .n = 40 }; break :blk v.p2(); }"}, .want = blk: {
            const V = struct {
                n: u8,
        ...
        .{ .src = &.{"blk: { const Vec = struct { x: u8, y: u8, fn dot(self: @This(), o: @This()) u8 { return self.x * o.x + self.y * o.y; } }; const p: Vec = .{ .x = 2, .y = 3 }; const q: Vec = .{ .x = 4, .y = 5 }; break :blk p.dot(q); }"}, .want = blk: {
            const Vec = struct {
                x: u8,
                y: u8,
