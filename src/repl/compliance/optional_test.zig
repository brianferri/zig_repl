...
test "compliance: optional values (payload, null, unwrap)" {
    try compliance.check(a, .{
        .{ .src = &.{"@as(?i32, 5)"}, .want = @as(?i32, 5) },
        .{ .src = &.{"@as(?i32, null)"}, .want = @as(?i32, null) },
        .{ .src = &.{"@as(?i32, 5).?"}, .want = @as(?i32, 5).? },
        .{
...
test "compliance: struct/union init through an optional result location" {
    try compliance.check(a, .{
        ...
        .{ .src = &.{"@as(?struct { a: u8, b: u8 }, .{ .a = 3, .b = 9 }).?.b"}, .want = @as(?struct { a: u8, b: u8 }, .{ .a = 3, .b = 9 }).?.b },
        .{ .src = &.{"@as(?union(enum) { x: u8, y: u16 }, .{ .x = 7 }).?.x"}, .want = @as(?union(enum) { x: u8, y: u16 }, .{ .x = 7 }).?.x },
        .{ .src = &.{"(@as(anyerror!struct { a: u8 }, .{ .a = 5 }) catch unreachable).a"}, .want = (@as(anyerror!struct { a: u8 }, .{ .a = 5 }) catch unreachable).a },
        .{ .src = &.{"@as(?struct { a: u8 = 4 }, .{}).?.a"}, .want = @as(?struct { a: u8 = 4 }, .{}).?.a },
    });
}
...
test "compliance: optional null test drives if-capture and orelse" {
    try compliance.check(a, .{
        ...
        .{ .src = &.{"blk: { const n: u32 = 5; break :blk if (n) |v| v else 0; }"}, .reject = {} },
    });
}
