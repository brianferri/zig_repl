...
test "compliance: switch dispatch, ranges, and multi-case items" {
    try compliance.check(a, &.{
        .{ .src = &.{"switch (1) { 0 => 100, 1 => 200, else => 999 }"}, .want = compliance.want(switch (1) {
            0 => 100,
            1 => 200,
            else => 999,
        }) },
        .{ .src = &.{"switch (5) { 0 => 100, else => 999 }"}, .want = compliance.want(switch (5) {
            0 => 100,
            else => 999,
        }) },
        .{ .src = &.{"switch (@as(u8, 2)) { 0 => 10, 1 => 20, 2 => 30, else => 0 }"}, .want = compliance.want(switch (@as(u8, 2)) {
            0 => 10,
            1 => 20,
            2 => 30,
            else => 0,
        }) },
        .{ .src = &.{"switch (3) { 0, 1, 2 => 100, 3, 4 => 200, else => 0 }"}, .want = compliance.want(switch (3) {
            0, 1, 2 => 100,
            3, 4 => 200,
            else => 0,
        }) },
        .{ .src = &.{"switch (0) { 0...2 => 100, 3...5 => 200, else => 999 }"}, .want = compliance.want(switch (0) {
            0...2 => 100,
            3...5 => 200,
            else => 999,
        }) },
        .{ .src = &.{"switch (5) { 0...2 => 100, 3...5 => 200, else => 999 }"}, .want = compliance.want(switch (5) {
            0...2 => 100,
            3...5 => 200,
            else => 999,
        }) },
        .{ .src = &.{"switch (7) { 0...2 => 100, 3...5 => 200, else => 999 }"}, .want = compliance.want(switch (7) {
            0...2 => 100,
            3...5 => 200,
            else => 999,
        }) },
...
test "compliance: anonymous and bound function types render" {
    try compliance.check(a, &.{
        .{ .src = &.{"fn () void"}, .want = compliance.want(fn () void) },
        .{ .src = &.{"fn (u32) u8"}, .want = compliance.want(fn (u32) u8) },
        .{ .src = &.{"fn (u32, i32) u8"}, .want = compliance.want(fn (u32, i32) u8) },
        .{ .src = &.{"fn (u32, bool) u8"}, .want = compliance.want(fn (u32, bool) u8) },
        .{
...
test "compliance: switch operand and exhaustiveness are validated" {
    try compliance.check(a, &.{
        .{ .src = &.{"blk: { const E = enum { a, b }; break :blk switch (E.b) { .a => 1, .b => 2 }; }"}, .want = compliance.want(blk: {
            const E = enum { a, b };
        ...
        .{ .src = &.{"blk: { const E = enum { a, b, c }; break :blk switch (E.c) { .a => 1, else => 9 }; }"}, .want = compliance.want(blk: {
            const E = enum { a, b, c };
        ...
        .{ .src = &.{"blk: { break :blk switch (@as(u1, 0)) { 0 => 10, 1 => 20 }; }"}, .want = compliance.want(blk: {
            break :blk switch (@as(u1, 0)) {
                0 => 10,
                1 => 20,
            };
        }) },
        .{ .src = &.{"blk: { break :blk switch (@as(u2, 3)) { 0...3 => 10 }; }"}, .want = compliance.want(blk: {
            break :blk switch (@as(u2, 3)) {
                0...3 => 10,
            };
        }) },
        .{ .src = &.{"blk: { break :blk switch (true) { true => 1, false => 0 }; }"}, .want = compliance.want(blk: {
            break :blk switch (true) {
                true => 1,
                false => 0,
            };
        }) },
        .{ .src = &.{"blk: { break :blk switch (@as(u8, 5)) { 0...4 => 1, 5...255 => 2 }; }"}, .want = compliance.want(blk: {
            break :blk switch (@as(u8, 5)) {
                0...4 => 1,
                5...255 => 2,
            };
        }) },
        .{ .src = &.{ "const E = error{ A, B };", "const x: E!u8 = error.A;", "x catch |e| switch (e) { error.A => 1, error.B => 2 }" }, .want = compliance.want(blk: {
            const E = error{ A, B };
        ...
        .{ .src = &.{"blk: { const x: ?u8 = 1; break :blk switch (x) { else => 0 }; }"}, .reject = true },
        .{ .src = &.{"blk: { const E = enum { a, b, c }; break :blk switch (E.a) { .a => 1, .b => 2 }; }"}, .reject = true },
        .{ .src = &.{"blk: { const U = union(enum) { a: u32, b: u8 }; const u = U{ .a = 1 }; break :blk switch (u) { .a => |v| v }; }"}, .reject = true },
        .{ .src = &.{"blk: { break :blk switch (@as(u8, 0)) { 0 => 10, 1 => 20 }; }"}, .reject = true },
        .{ .src = &.{"blk: { break :blk switch (true) { true => 1 }; }"}, .reject = true },
        .{ .src = &.{"blk: { const E = enum { a, b }; const k = E.b; break :blk switch (E.a) { k => 1 }; }"}, .reject = true },
        .{ .src = &.{"blk: { const E = enum { a, b }; break :blk switch (E.a) { .a => 1, .a => 2, .b => 3 }; }"}, .reject = true },
        .{ .src = &.{"blk: { const E = enum { a, b }; break :blk switch (E.a) { .a => 1, .b => 2, _ => 3 }; }"}, .reject = true },
        .{ .src = &.{ "const E = error{ A, B };", "const x: E!u8 = error.A;", "x catch |e| switch (e) { error.A => 1 }" }, .reject = true },
    });
...
test "compliance: switch else and scalar-prong captures bind the operand" {
    try compliance.check(a, &.{
        .{ .src = &.{"blk: { const x: u32 = 48; break :blk switch (x) { 0 => @as(u32, 0), else => |m| 1 + m }; }"}, .want = compliance.want(blk: {
            const x: u32 = 48;
            break :blk switch (x) {
                0 => @as(u32, 0),
                else => |m| 1 + m,
        ...
        .{ .src = &.{"blk: { const x: u8 = 5; break :blk switch (x) { 1, 2 => |v| @as(u8, v) * 10, else => |v| v }; }"}, .want = compliance.want(blk: {
            const x: u8 = 5;
            break :blk switch (x) {
                1, 2 => |v| @as(u8, v) * 10,
                else => |v| v,
        ...
        .{ .src = &.{"blk: { const E = enum { a, b, c }; const v: E = .c; break :blk switch (v) { .a => @as(u8, 1), else => |e| @intFromEnum(e) }; }"}, .want = compliance.want(blk: {
            const E = enum { a, b, c };
            const v: E = .c;
            break :blk switch (v) {
                .a => @as(u8, 1),
...
test "compliance: a non-exhaustive switch names each unhandled value" {
    // A union's tag enum is generated, so an unhandled tag resolves through the owning union's field.
