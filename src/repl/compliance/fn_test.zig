...
test "compliance: a call resolves a fn-pointer field, binding no receiver" {
    ...
    try compliance.check(a, &.{
        ...
        .{ .src = &.{"blk: { const S = struct { f: *const fn (u8) u8 }; const s: S = .{ .f = struct { fn dbl(x: u8) u8 { return x *% 2; } }.dbl }; const p = &s; break :blk p.f(3); }"}, .want = compliance.want(blk: {
            const S = struct { f: *const fn (u8) u8 };
            const s: S = .{ .f = struct {
                fn dbl(x: u8) u8 {
                    return x *% 2;
        ...
        .{ .src = &.{"blk: { const U = union(enum) { f: *const fn (u8) u8, g: u8 }; const u: U = .{ .f = struct { fn inc(x: u8) u8 { return x +% 1; } }.inc }; break :blk u.f(4); }"}, .want = compliance.want(blk: {
...
test "compliance: a later arg's result type resolves from an earlier comptime param" {
    try compliance.check(a, &.{
        ...
        .{ .src = &.{"blk: { const F = struct { fn f(comptime T: type, x: T) T { return x; } }; break :blk F.f(u32, @intCast(@as(u64, 7))); }"}, .want = compliance.want(blk: {
            const F = struct {
        ...
        .{ .src = &.{"blk: { const F = struct { fn f(x: f64) i32 { const b: comptime_int = @sizeOf(@TypeOf(x)); return @as(i32, 5) - b; } }; break :blk F.f(2.0); }"}, .want = compliance.want(blk: {
            const F = struct {
                fn f(x: f64) i32 {
        ...
        .{ .src = &.{ "const std = @import(\"std\");", "std.math.frexp(@as(f64, 2.0)).exponent" }, .want = compliance.want(std.math.frexp(@as(f64, 2.0)).exponent) },
        .{ .src = &.{ "const std = @import(\"std\");", "std.math.pow(f64, 2.0, 10.0)" }, .want = compliance.want(std.math.pow(f64, 2.0, 10.0)) },
    });
}
